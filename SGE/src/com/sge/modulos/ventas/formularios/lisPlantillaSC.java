...
public class lisPlantillaSC extends frameBase<PlantillaSolicitudCotizacion> {
    ...
    public void Init() {
        switch (getModo()) {
            case 0:
                OcultarColumnas(tbPlantillas, new int[]{0, 1});
                OcultarControl(btnSeleccionar);
                break;
            case 2:
                OcultarColumnas(tbPlantillas, new int[]{1, 5, 6});
        ...
        protected void done() {
            try {
                ...
                if (resultado[0].equals("true")) {
                    ...
                    if (getModo() == 2 && plantillas.length == 1) {
                        setSeleccionados(new ArrayList<>());
                        getSeleccionados().add(plantillas[0]);
                        Cerrar();
                    } else {
    ...
    private void btnSeleccionarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSeleccionarActionPerformed
        ...
        switch (getModo()) {
            case 2:
                setSeleccionados(new ArrayList<>());
                for (int i = 0; i < tbPlantillas.getRowCount(); i++) {
                    boolean check = ObtenerValorCelda(tbPlantillas, i, 0);
                    if (check) {
                        PlantillaSolicitudCotizacion plantilla = new PlantillaSolicitudCotizacion();
                        plantilla.setIdPlantillaSolicitudCotizacion(ObtenerValorCelda(tbPlantillas, i, 1));
                        plantilla.setNombre(ObtenerValorCelda(tbPlantillas, i, 2));
                        plantilla.setLineaProduccion(ObtenerValorCelda(tbPlantillas, i, 3));
                        getSeleccionados().add(plantilla);
                    }
                }
                Cerrar();
                break;
